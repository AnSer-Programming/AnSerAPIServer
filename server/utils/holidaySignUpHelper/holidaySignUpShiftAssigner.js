  ...
  for (let x = 0; x < agents.length; x++) {
    ...
    if (requestedShifts.length > 0) {
      for (let y = 0; y < requestedShifts.length; y++) {
        if (agents[x].EmployeeID == requestedShifts[y].employee_id) {
      ...
      if (firstPick || secondPick) {
        if (firstPick && overviewData[`${firstPick}`].availableShifts > 0) {
          ...
          setShifts[setShifts.length] = [firstPick, agents[x].Agent_name];
          ...
          setShifts[setShifts.length] = [secondPick, agents[x].Agent_name];
        }
      }
    }
  }
