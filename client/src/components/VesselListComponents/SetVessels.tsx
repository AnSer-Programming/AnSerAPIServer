...
const SetVessels = (data: any) => {
  ...
  const getContacts = async (accountNum:number) => {
    ...
    try {
      ...
      for(let i = 0; i < data.length; i++) {
        if(data[i].Name) {
          if(!data[i].Name.includes('Office')) {
            contactsArray[count] = data[i].Name;
            count++;
          }
        }
      }
    ...
    return await contactsArray.sort();
  }
    ...
    const options = async () => {
      switch (data.accountNum) {
        case '38':
          ...
          break;
        case '6071':
          setOptions(await getContacts(6071));
          // setOptions(
